...
void StudentService::full_update_student(int id, string name, int age, string department, float gpa)
{
    string query = "UPDATE students SET ";

    query +=  "name = '" + name + "', age = " + to_string(age);
    query +=  ",department = '" + department + "', gpa = " + to_string(gpa);
    query += " WHERE student_id = " + to_string(id);
    query += ";";

    if(db.execute_query(query.c_str()))
    {
        cout << endl << "Couldn't fully update student: " << id << endl;
        cout << endl;
        return;
    }
...
vector<Student> StudentService::search_student_in_field(string column_name, string search_value){
    vector<Student> students;
    string query;
    if(column_name == "name" || column_name == "department" || column_name == "gpa"){
        query = "SELECT * FROM students WHERE " + column_name + " like '" + search_value + "%';";
    }
    else
    {
        query = "SELECT * FROM students WHERE " + column_name + " = " + search_value + ";";
    }

    if(db.execute_query(query.c_str()))
    {
        cout << endl << "Couldn't search for students with search value: " << search_value << " in database" << endl;
        cout << endl;
        return students;
    }
    ...
    if(students.empty())
    {
        cout << "There are no students in the database with this search value: " << search_value << endl;
    }
    return students;
}
...
string StudentService::get_highest_gpa()
{
    string query = "SELECT MAX(GPA) FROM students;";

    if(db.execute_query(query.c_str()))
    {
        cout << endl << "Couldn't get highest gpa" << endl;
        cout << endl;
        return "0";
    }
    ...
    {
        cout << "Couldn't find any highest GPA" << endl;
        cout << endl;
        return "0";
    }
}

string StudentService::get_lowest_gpa()
{
    string query = "SELECT MIN(GPA) FROM students;";

    if(db.execute_query(query.c_str()))
    {
        cout << endl << "Couldn't get lowest gpa" << endl;
        cout << endl;
        return "0";
    }
    ...
    {
        cout << "Couldn't find any lowest GPA" << endl;
        cout << endl;
        return "0";
    }
...
string StudentService::get_students_count()
{
    string query = "SELECT COUNT(student_id) FROM students;";

    if(db.execute_query(query.c_str()))
    {
        cout << endl << "Couldn't get students count" << endl;
        cout << endl;
        return "0";
    }
    ...
    {
        cout << "Couldn't find any students count" << endl;
        cout << endl;
        return "0";
    }
